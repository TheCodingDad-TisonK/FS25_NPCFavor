 ...
 * QUICK START:
 *   cd translations/
 *   node lang_sync.js sync      # Sync all languages
 *   node lang_sync.js status    # Quick overview
 *   node lang_sync.js report    # Detailed breakdown
 *   node lang_sync.js help      # Full documentation
 *
...
function autoDetectFilePrefix() {
    ...
    if (fs.existsSync(`lang_${CONFIG.sourceLanguage}.xml`)) return 'lang';
    ...
    const files = fs.readdirSync('.');
    for (const file of files) {
        if (file.match(/^lang_[a-z]{2}\.xml$/i)) return 'lang';
...
function syncTranslations() {
    ...
    if (!filePrefix) {
        ...
        console.error(`Looking for: lang_${CONFIG.sourceLanguage}.xml, translation_${CONFIG.sourceLanguage}.xml, or l10n_${CONFIG.sourceLanguage}.xml`);
        process.exit(1);
    }
    ...
    console.log(`      Format: ${format}${format === 'texts' ? ' (no hash support - stale detection unavailable)' : ''}`);
    console.log();
    ...
    if (format === 'elements') {
        ...
        console.log("Using 'texts' XML format (no hash embedding).");
        console.log("Missing keys, duplicates, orphans, and format errors are detected.");
        console.log("For stale detection, consider migrating to 'elements' format.");
    }
    console.log();
...
function checkSync() {
    ...
    if (hasProblems) {
        ...
        if (totalMissing > 0) {
            console.log("CRITICAL: Missing keys detected! Run 'node lang_sync.js sync' to fix.");
        }
...
function showStatus() {
    ...
    console.log(`Source: ${sourceFile} (${sourceEntries.size} keys)`);
    console.log(`Format: ${format}${format === 'elements' ? ' (hash-enabled)' : ' (no hash support)'}`);
    console.log();
...
USAGE:
  node lang_sync.js sync     # Sync all languages, add missing keys
  node lang_sync.js check    # Verify sync status
  node lang_sync.js status   # Quick overview table
  node lang_sync.js report   # See detailed stale/missing lists
